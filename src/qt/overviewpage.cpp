...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::LDB)
    {
    }
...
//    ui->label_ldbcoin4->setVisible(showImmature || showWatchOnlyImmature);
...
//        model->getWatchBalance(), model->getWatchUnconfirmedBalance(), model->getWatchImmatureBalance());
        ...
        connect(ui->blabel_ldbcoin, SIGNAL(clicked()), this, SLOT(openMyAddresses()));

        emit model->makeBalance();
    }

    // update the display unit, to not use the default ("ldbcoin")
    updateDisplayUnit();
}
...
void OverviewPage::updateMasternodeInfo()
{
  ...
  if (masternodeSync.IsBlockchainSynced() && masternodeSync.IsSynced())
  {
   ...
   std::vector<CMasternode> vMasternodes = mnodeman.GetFullMasternodeMap();
    ...
    if (CurrentBlock >= 0) {
        ...
        ui->roi_22->setText(mn2==0 ? " " : QString::number( 26000/roi2,'f',1).append(" days"));
        ui->roi_32->setText(mn3==0 ? " " : QString::number( 56000/roi3,'f',1).append(" days"));
        ui->roi_42->setText(mn4==0 ? " " : QString::number(160000/roi4,'f',1).append(" days"));
    }
    CAmount tNodesSumm = mn1*5000 + mn2*26000 + mn3*56000 + mn4*160000;
  ...
  if (CurrentBlock >= 0) {
      ui->label_lcolat->setText("5000 LDB");
      ui->label_mcolat->setText("26000 LDB");
      ui->label_fcolat->setText("56000 LDB");
      ui->label_pcolat->setText("160000 LDB");
  }
...
void OverviewPage::updateBlockChainInfo()
{
    if (masternodeSync.IsBlockchainSynced())
    {
        ...
        double BlockReward = GetBlockValue(CurrentBlock);
        double BlockRewardldbcoin =  static_cast<double>(BlockReward/COIN);
        ...
        ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardldbcoin, 'f', 1).append(" | ") + QString::number(GetSporkValue(SPORK_11_DEV_FEE)).append("%"));

        ui->label_Supply_value->setText(QString::number(chainActive.Tip()->nMoneySupply / COIN).append(" LDB"));
...
void OverviewPage::on_pushButton_Website_clicked() {
    QDesktopServices::openUrl(QUrl("https://landun.pro/", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Discord_clicked() {
    QDesktopServices::openUrl(QUrl("https://jq.qq.com/?_wv=1027&k=5baqeWP", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Telegram_clicked() {
   QDesktopServices::openUrl(QUrl("https://#", QUrl::TolerantMode));
}
...
void OverviewPage::on_pushButton_Explorer_clicked() {
    QDesktopServices::openUrl(QUrl("https://explorer.landun.pro/", QUrl::TolerantMode));
}
