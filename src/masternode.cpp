...
unsigned CMasternode::Level(CAmount vin_val, int blockHeight)
{
    if (blockHeight >= 0 ) {
      switch(vin_val) {
          ...
          case 26000 * COIN: return 2;
          case 56000 * COIN: return 3;
          case 160000 * COIN: return 4;
      }
    }
    return 0;
}
...
*/
    ...
    // should be at least not earlier than block when 1000 ldbcoin tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && mi->second) {
        CBlockIndex* pMNIndex = mi->second;                                                        // block for 1000 ldbcoin tx -> 1 confirmation
