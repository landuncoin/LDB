...
#if defined(NDEBUG)
#error "ldbcoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uldbcoin) are considered zero fee (for relaying and mining)
...
CAmount GetBlockValue(int nHeight)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET){
        if ( nHeight == 0 ) {
            nSubsidy = 30000000 * COIN; // premine
        } else if (nHeight < 300) {
            nSubsidy = 200 * COIN;
        ...
        if ( nHeight == 0 ) {
            ...
            nSubsidy = 30000 * COIN;
        } else if (nHeight >= 301 && nHeight < 6000) {
            nSubsidy = 5 * COIN;
        } else if (nHeight >= 6000 && nHeight < 30000) {
            nSubsidy = 10 * COIN;
